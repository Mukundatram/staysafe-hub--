...
// Extracted component for document preview using authenticated download
const DocumentPreviewSection = ({ document }) => {
  const { previewUrl, loading, error } = useDocumentPreview(document?._id);
                ...
                <div className="modal-overlay" onClick={() => setShowDocumentModal(false)}>
                  ...
                  >
                    ...
                    <div className="modal-body">
                      ...
                      {/* Metadata Section (OCR / College Info) */}
                      {selectedDocument.metadata && Object.keys(selectedDocument.metadata).length > 0 && (
                        <div className="detail-section" style={{ background: 'var(--bg-tertiary)', padding: '16px', borderRadius: '12px' }}>
                          <h3><HiOutlineClipboardList size={18} /> Extracted Data / Metadata</h3>

                          {selectedDocument.metadata.collegeName && (
                            <div className="detail-row">
                              <span className="detail-label">Claimed College:</span>
                              <span className="detail-value" style={{ fontWeight: 600 }}>{selectedDocument.metadata.collegeName}</span>
                            </div>
                          )}

                          {selectedDocument.metadata.graduationYear && (
                            <div className="detail-row">
                              <span className="detail-label">Claimed Grad Year:</span>
                              <span className="detail-value">{selectedDocument.metadata.graduationYear}</span>
                            </div>
                          )}

                          {selectedDocument.metadata.automatedVerification && (
                            <div className="detail-row">
                              <span className="detail-label">Auto-Verify Status:</span>
                              <Badge variant={
                                selectedDocument.metadata.automatedVerification === 'success' ? 'success' :
                                  selectedDocument.metadata.automatedVerification.includes('failed') ? 'danger' : 'warning'
                              }>
                                {selectedDocument.metadata.automatedVerification.replace(/_/g, ' ')}
                              </Badge>
                            </div>
                          )}

                          {selectedDocument.metadata.extractedText && (
                            <div className="detail-row" style={{ flexDirection: 'column', alignItems: 'flex-start', marginTop: '12px' }}>
                              <span className="detail-label" style={{ marginBottom: '8px' }}>Raw OCR Text Extracted:</span>
                              <pre style={{
                                width: '100%',
                                maxHeight: '150px',
                                overflowY: 'auto',
                      ...
                      <DocumentPreviewSection document={selectedDocument} />
