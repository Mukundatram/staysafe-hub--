...
// Ensure Cloudinary is configured (if not already globally configured)
cloudinary.config({
...
// Configure Cloudinary Storage for sensitive documents
const storage = new CloudinaryStorage({
  cloudinary: cloudinary,
  params: {
    folder: 'staysafe/documents',
    allowed_formats: ['jpg', 'png', 'jpeg', 'pdf'],
    transformation: [{ width: 1200, height: 1200, crop: 'limit' }], // Maintain high quality for OCR
    // Mark as private/authenticated so it doesn't leak publicly on search engines
    type: 'private'
  }
});
